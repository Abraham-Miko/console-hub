<x-app-layout>
    ...
    <script>
        function initModalEdit(pemesananId, selectedUnitId, allUnitsData) {
            ...
            const jenisPeralatanSelectEdit = document.getElementById('jenis_peralatan_id-' + pemesananId);
            const unitPeralatanSelectEdit = document.getElementById('unit_peralatan_id-' + pemesananId);

            if (jenisPeralatanSelectEdit && jenisPeralatanSelectEdit.value) {
                // Memastikan nilai awal sudah terbaca (misalnya '8')
                jenisPeralatanSelectEdit.dispatchEvent(new Event('change'));
            }
            jenisPeralatanSelectEdit.addEventListener('change', function() {
                const selectedJenisId = this.value;

                unitPeralatanSelectEdit.innerHTML = '<option value="" disabled selected>Memuat Unit...</option>';
                unitPeralatanSelectEdit.disabled = true;

                if (!selectedJenisId) return;

                const filteredUnits = availableUnits.filter(unit => {
                    return String(unit.id_jenis_peralatan) === selectedJenisId;
                });


                if (filteredUnits.length > 0) {
                    unitPeralatanSelectEdit.innerHTML = '<option value="" disabled selected>Pilih </option>';

                    filteredUnits.forEach(unit => {
                        const option = document.createElement('option');
                        option.value = unit.id_unit_peralatan;
                        const warna = unit.warna ? ` (${unit.warna})` : '';
                        option.textContent = unit.nomor_seri + warna;

                        if (String(unit.id_unit_peralatan) === selectedUnitIdOld) {
                            option.selected = true;
                        }

                        // console.log(unit.plat_nomor);

                        unitPeralatanSelectEdit.appendChild(option);
                    });
                    // console.log(selectedUnitId);
                    unitPeralatanSelectEdit.disabled = false;

                } else {
                    unitPeralatanSelectEdit.innerHTML = '<option value="" disabled selected>Tidak Ada Unit Tersedia</option>';
                }
            });

            if (jenisPeralatanSelectEdit.value) {
                jenisPeralatanSelectEdit.dispatchEvent(new Event('change'));
            }
            ...
            function calculateTotalHarga() {
                ...
                if (selectedUnit && selectedUnit.jenis_peralatan) {
                    hargaPerHari = selectedUnit.jenis_peralatan.harga_rental_per_hari;
                }
        ...
        function initModalCreate() {
            ...
            const jenisPeralatanSelect = document.getElementById('jenis_peralatan_id');
            const unitPeralatanSelect = document.getElementById('unit_peralatan_id');

            jenisPeralatanSelect.addEventListener('change', function() {
                const selectedJenisId = this.value;

                unitPeralatanSelect.innerHTML = '<option value="" disabled selected>Memuat Unit...</option>';
                unitPeralatanSelect.disabled = true;
                ...
                if (filteredUnits.length > 0) {
                    unitPeralatanSelect.innerHTML = '<option value="" disabled selected>Pilih Unit</option>';
                    filteredUnits.forEach(unit => {
                        const option = document.createElement('option');
                        option.value = unit.id_unit_peralatan;
                        const warna = unit.warna ? ` (${unit.warna})` : ''; // Cek jika data warna ada
                        option.textContent = unit.nomor_seri + warna;
                        unitPeralatanSelect.appendChild(option);
                    });
                    unitPeralatanSelect.disabled = false;
                } else {
                    unitPeralatanSelect.innerHTML = '<option value="" disabled selected>Tidak Ada Unit Tersedia</option>';
                }
            });

            // Jalankan event 'change' saat halaman dimuat jika ada nilai old()
            if (jenisPeralatanSelect.value) {
                jenisPeralatanSelect.dispatchEvent(new Event('change'));
            }
            ...
            function calculateTotalHarga() {
                ...
                const selectedUnit = unitTersedia.find(unit => String(unit.id_unit_peralatan) === selectedUnitId);
                    ...
                    <table id="search-table" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-300 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3">Unit Peralatan</th>
                        ...
                        <tbody>
                            ...
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td class="px-6 py-4">
                                    <a href="{{ route('pemesanan.qr.konversi', ['token' => $pemesanan->verification_token]) }}" class="font-medium text-green-600 dark:text-green-500 hover:text-green-400 p-1.5 rounded-full inline-flex items-center" title="Download QR Code">
                                ...
                                <td class="px-6 py-4">{{ $pemesanan->unitPeralatan->jenis_peralatan->merek }} [{{ $pemesanan->unitPeralatan->nomor_seri }}]</td>
